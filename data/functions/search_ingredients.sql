...
-- Created: 2025-01-17
-- Updated: 2025-01-28 - Updated limit to 250 and unified rate limiting
...
-- 4. Pagination support (250 ingredients per page)
-- 5. Automatic logging with subscription and rate limit metadata
--
-- Features:
...
-- - Limited results: Maximum 250 results per search strategy
-- - Graceful rate limits: Returns special response instead of throwing errors
-- - Unified rate limiting: Uses 'search' action type for both product and ingredient searches
...
-- Free: 10 searches per day (unified with product searches)
-- Standard: unlimited searches per day
-- Premium: unlimited searches per day
...
-- Search Strategy:
-- 1. Exact match: i.title = 'search term'
-- 2. Starts with: i.title ILIKE 'search term%'
-- 3. Contains: i.title ILIKE '%search term%'
-- 4. Returns 250 results per strategy with hierarchical fallback
-- 5. Results ordered by ingredient title for consistent pagination
