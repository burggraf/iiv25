	...
	useEffect(() => {
		...
		const handleJobCompleted = async (job: BackgroundJob) => {
			...
			try {
				...
				const notification: JobNotification = {
					...
					job,
					product,
					message: getSuccessMessage(job.jobType),
					type: 'success',
					timestamp: new Date(),
				}
		...
		const handleJobFailed = (job: BackgroundJob) => {
			...
			const notification: JobNotification = {
				...
				job,
				product: null,
				message: getErrorMessage(job.jobType, job),
				type: 'error',
				timestamp: new Date(),
			}
			...
			if (nextAppState === 'active' && pendingJobResults.size > 0) {
				...
				pendingJobResults.forEach(({ job, product }, jobId) => {
					const notification: JobNotification = {
						...
						job,
						product,
						message: job.status === 'completed' ? getSuccessMessage(job.jobType) : getErrorMessage(job.jobType, job),
...
function getSuccessMessage(jobType: string): string {
	switch (jobType) {
...
function getErrorMessage(jobType: string, job?: BackgroundJob): string {
	if (job?.errorMessage) {
